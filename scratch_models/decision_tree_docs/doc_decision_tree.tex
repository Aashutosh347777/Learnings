...
Decision is one of the classic problem which uses \textbf{Recursion} to find the splitting criteria and build untill a base criteria is met.
...
Lets classify the Components hierarchialy, namely from \textbf{Classes}, as it sits on the top of the objects and serves as a blue print.
...
\begin{table} [H]
    ...
    \begin{tabularx}{\linewidth}{|X|X|X|X|X|}
        ...
        \hline
        DecisionTree (DT) & Independent & Provides base class for Classification and Regression Tasks.& depth, max\_depth, min\_entropy & build\_tree \\
        \hline
        Node & Independent & Provide branching/ endpoint in the Tree. & value, left, right, feature, threshold & None \\
        \hline
        CriteriaFinder (CF) & Independent & Compute mathematical aspects for the DT & sys\_entropy, feat\_entropy, ig, probs & find\_criteria, cal\_probs, cal\_entropy, cal\_ig \\
        \hline
        DecisionTreeClassifier (DTC) & Inherited DT & DT Specialized for handling classification tasks. & None & None \\
        \hline
        DecisionTreeClassifier (DTC) & Inherited DT & Supposed to hold the data that arrives for being & num cols & shuffle \\
        \hline
    \end{tabularx}
    \caption{Independent Components}
\end{table}

\section{Histogram Binning, Move to a different document}
